...
local musicPanel = nil -- Spotify Music Panel (created later)
...
local siriusValues = {
	...
	executors = {"synapse x", "script-ware", "krnl", "scriptware", "comet", "valyse", "fluxus", "electron", "hydrogen", "xeno", "solara", "bunni", "bunni.lol", "wave", "volt"},
...
local siriusSettings = {
	{
		...
		categorySettings = {
			{
				name = 'Dynamic Island',
				description = 'Enable the Dynamic Island music player at the top of the screen.',
				settingType = 'Boolean',
				current = true,
				id = 'dynamicisland'
			},
...
-- local musicPanel = UI.Music -- OLD MUSIC PANEL - NOW USING SPOTIFY UI INSTEAD
...
local clientCardDefaults = nil
local supportedExecutorGradient = ColorSequence.new({
	ColorSequenceKeypoint.new(0, Color3.fromRGB(70, 170, 105)), -- richer green lead
	ColorSequenceKeypoint.new(0.4, Color3.fromRGB(25, 90, 45)), -- dark green mid
	ColorSequenceKeypoint.new(0.55, Color3.fromRGB(8, 25, 12)), -- pivot toward black just past half
	ColorSequenceKeypoint.new(1, Color3.fromRGB(0, 0, 0)) -- strong black tail
})
local supportedExecutorStrokeGradient = ColorSequence.new({
	ColorSequenceKeypoint.new(0, Color3.fromRGB(40, 130, 80)),
	ColorSequenceKeypoint.new(1, Color3.fromRGB(8, 30, 15))
})
local spamGuard = {
	keywords = {
		{"blox", "pink", "robux"},
...
function spamGuard:destroy(obj)
	...
	local container = obj:FindFirstAncestorWhichIsA("Frame")
	if container then
...
function spamGuard:clearChat()
	...
	if tcs and tcs.ChatVersion == Enum.ChatVersion.TextChatService then
		...
		if general then pcall(general.SendAsync, general, "/clear") end
		return
	end
	...
	if say then pcall(function() say:FireServer("/clear", "All") end) end
end
...
function spamGuard:report(userId)
	...
	pcall(function()
		players:ReportAbuse(target, "Scamming", "Sirius auto-report: spam/scam message detected.")
	end)
end
...
function spamGuard:setupScrubber()
	...
	self.setup = true
	task.spawn(function()
		local chatGui = coreGui:WaitForChild("Chat", 5)
		if not chatGui then return end
		...
		local logFrame = nil
		for _, desc in ipairs(chatGui:GetDescendants()) do
			if desc.Name == "Frame_MessageLogDisplay" and desc:FindFirstChild("ScrollingFrame") then
				logFrame = desc:FindFirstChild("ScrollingFrame")
				break
			end
		end
		if logFrame then
			...
			logFrame.ChildAdded:Connect(function(child)
				task.defer(function()
					self:inspect(child)
				end)
			end)
...
if getMessage then
	getMessage.OnClientEvent:Connect(function(packet, channel)
		...
		local notifySetting = checkSetting("Spam Notifications")
		if notifySetting and notifySetting.current then
			spamGuard:notify()
		end

		local clearSetting = checkSetting("Clear Chat on Spam")
		if clearSetting and clearSetting.current then
			spamGuard:clearChat()
		end

		local reportSetting = checkSetting("Auto-Report Spam")
		if reportSetting and reportSetting.current and packet and packet.SpeakerUserId then
			spamGuard:report(packet.SpeakerUserId)
		end
...
-- Filter spam via TextChatService (new chat)
do
	local tcs = game:FindFirstChildOfClass("TextChatService")
	if tcs then
		tcs.OnIncomingMessage = function(message)
			local antiSpam = checkSetting("Anti-Spam Bot")
			if not (antiSpam and antiSpam.current) or not message then
				return Enum.TextChatMessageResult.Show
			end

			if spamGuard:isSpam(message.Text or "") then
				spamGuard:setupScrubber()
				...
				local reportSetting = checkSetting("Auto-Report Spam")
				if reportSetting and reportSetting.current and message.TextSource then
					spamGuard:report(message.TextSource.UserId)
				end
				...
				return Enum.TextChatMessageResult.Hidden
			end

			return Enum.TextChatMessageResult.Show
		end

		-- Fallback listener (some executors block OnIncomingMessage override)
...
local function ensureFrameProperties()
	...
	-- Music Panel will be dynamically created with new Spotify UI
	if UI:FindFirstChild("Music") then UI.Music.Visible = false end -- Hide old music panel
...
-- Dynamic Island (Apple-style mini player) - NEW IMPLEMENTATION
...
local function updateDynamicIsland(data)
...
-- Spotify Update Loop Variables
local spotifyUpdateRoutine = nil
local spotifyInterval = 1 -- Update every 1 second
...
-- Clean up on disable
local function stopSpotifyUpdateLoop()
	spotifyEnabled = false
	if spotifyUpdateRoutine then
		task.cancel(spotifyUpdateRoutine)
...
if getcustomasset then
	buildSpotifyUI()
	-- queueNotification("Spotify Player", "Music panel loaded! Click the Music button to open.", 9622474485)
else
...
local function UpdateHome()
	...
	local clientCard = homeContainer.Interactions.Client
	local clientGradient = clientCard:FindFirstChild("UIGradient")
	local clientStroke = clientCard:FindFirstChild("UIStroke")
	local clientStrokeGradient = clientStroke and clientStroke:FindFirstChild("UIGradient")

	if not clientCardDefaults then
		clientCardDefaults = {
			gradient = clientGradient and clientGradient.Color,
			strokeGradient = clientStrokeGradient and clientStrokeGradient.Color,
			strokeColor = clientStroke and clientStroke.Color,
			subtitleColor = clientCard.Subtitle.TextColor3,
			rotation = clientGradient and clientGradient.Rotation,
			strokeRotation = clientStrokeGradient and clientStrokeGradient.Rotation
		}
	end

	homeContainer.Interactions.Client.Title.Text = executorName
	local isSupportedExecutor = table.find(siriusValues.executors, string.lower(executorName)) ~= nil

	if isSupportedExecutor then
		clientCard.Subtitle.Text = "Your executor is supported"
		if clientGradient then
			clientGradient.Color = supportedExecutorGradient
			clientGradient.Rotation = ((clientCardDefaults and clientCardDefaults.rotation) or 0) + 180 -- flip so green leads, black trails
		end
		if clientStroke then clientStroke.Color = Color3.fromRGB(30, 215, 96) end
		if clientStrokeGradient then
			clientStrokeGradient.Color = supportedExecutorStrokeGradient
			clientStrokeGradient.Rotation = ((clientCardDefaults and clientCardDefaults.strokeRotation) or 0) + 180
		end
		clientCard.Subtitle.TextColor3 = Color3.fromRGB(200, 255, 210)
	else
		clientCard.Subtitle.Text = "This executor is not verified as supported - but may still work just fine."
		if clientCardDefaults then
			if clientGradient and clientCardDefaults.gradient then clientGradient.Color = clientCardDefaults.gradient end
			if clientGradient and clientCardDefaults.rotation then clientGradient.Rotation = clientCardDefaults.rotation end
			if clientStroke and clientCardDefaults.strokeColor then clientStroke.Color = clientCardDefaults.strokeColor end
			if clientStrokeGradient and clientCardDefaults.strokeGradient then clientStrokeGradient.Color = clientCardDefaults.strokeGradient end
			if clientStrokeGradient and clientCardDefaults.strokeRotation then clientStrokeGradient.Rotation = clientCardDefaults.strokeRotation end
			clientCard.Subtitle.TextColor3 = clientCardDefaults.subtitleColor or clientCard.Subtitle.TextColor3
		end
	end
...
local function onChatted(player, message)
	...
	-- Block common spam bot promos (e.g., blox.pink robux reward) locally
	local antiSpam = checkSetting("Anti-Spam Bot")
	if antiSpam and antiSpam.current and spamGuard:isSpam(message) then
		spamGuard:setupScrubber()
		if (checkSetting("Spam Notifications") or {}).current ~= false then
			spamGuard:notify()
		end
		if (checkSetting("Clear Chat on Spam") or {}).current then
			spamGuard:clearChat()
		end
		if (checkSetting("Auto-Report Spam") or {}).current and player then
			spamGuard:report(player.UserId)
		end
		task.defer(function()
			for i = 1, 12 do
				task.wait(0.05 * i)
...
--[[ OLD MUSIC PANEL EVENTS - DISABLED FOR SPOTIFY INTEGRATION
...
smartBar.Buttons.Music.Interact.MouseButton1Click:Connect(function()
	...
	if musicPanel then
		if musicPanel.Visible then
			-- Close (Settings Style)
			debounce = true
			
			-- Hide content immediately to prevent visual glitches during shrink
			if musicPanel:FindFirstChild("TokenSection") then musicPanel.TokenSection.Visible = false end
			if musicPanel:FindFirstChild("ContentArea") then musicPanel.ContentArea.Visible = false end
			
			-- Shrink vertically and Fade Out (Matches Settings Panel)
			tweenService:Create(musicPanel, TweenInfo.new(0.5, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
			...
			-- Open (Synced Smooth Fade + Scale using CanvasGroup)
			debounce = true
			musicPanel.Visible = true
			musicPanel.Size = UDim2.new(0, 550, 0, 310) -- Start slightly smaller
			musicPanel.GroupTransparency = 1 -- Start invisible
			
			-- 1. Hide Input Elements Initially (For Delayed Fade)
			local inputElements = {}
			if musicPanel.TokenSection:FindFirstChild("SubmitButton") then table.insert(inputElements, musicPanel.TokenSection.SubmitButton) end
			if musicPanel.TokenSection:FindFirstChild("TokenInput") then table.insert(inputElements, musicPanel.TokenSection.TokenInput) end
			if musicPanel.TokenSection:FindFirstChild("HowButton") then table.insert(inputElements, musicPanel.TokenSection.HowButton) end
			
			for _, el in ipairs(inputElements) do
				el.Visible = false -- CRITICAL: Ensures text is hidden
				el.BackgroundTransparency = 1
				el.TextTransparency = 1
			end
			
			-- 2. Animate Scale
			tweenService:Create(musicPanel, TweenInfo.new(0.4, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {Size = UDim2.new(0, 600, 0, 350)}):Play()
			
			-- 3. Animate Group Fade In (Perfectly Synced)
			tweenService:Create(musicPanel, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {GroupTransparency = 0}):Play()
			
			startSpotifyUpdateLoop()
			
			-- Ensure ContentArea is visible if we have a token
			if userToken then
				musicPanel.ContentArea.Visible = true
				musicPanel.TokenSection.Visible = false
			else
				musicPanel.ContentArea.Visible = false
				musicPanel.TokenSection.Visible = true
				
				-- 4. Delayed Fade for Input Elements
				task.delay(0.3, function()
					for _, el in ipairs(inputElements) do
						el.Visible = true -- Reveal now
						
						local targetBg = 0
						if el.Name == "HowButton" then targetBg = 1 end
						if el.Name == "TokenInput" then targetBg = 0 end 
						if el.Name == "SubmitButton" then targetBg = 0 end
						
						tweenService:Create(el, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
